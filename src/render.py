...
def _layout_lines(
    ...
    uniform_rect_height: int | None = None,
    ...
    for line in lines:
        ...
        rect_h = uniform_rect_height if uniform_rect_height is not None else text_h + _PAD_Y * 2
        ...
        texts.append((text_x, text_y, clean_line, font))
        y += rect_h + _LINE_GAP
    end_y = y - _LINE_GAP if lines else start_y
    return rects, texts, end_y


def _compute_uniform_rect_height(draw: ImageDraw.ImageDraw, line_specs: list[tuple[str, any]]) -> int:
    max_text_h = 0
...
def _render_text_block(
    ...
    start_y: int,
    highlight_color: tuple,
    uniform_rect_height: int,
) -> tuple[list[tuple], list[tuple]]:
    rects: list[tuple] = []
    texts: list[tuple] = []

    t_rects, t_texts, title_end_y = _layout_lines(
        draw, title_lines, title_font, start_y, highlight_color, uniform_rect_height
    )
    ...
    if subtitle_lines:
        s_rects, s_texts, _ = _layout_lines(
            draw, subtitle_lines, subtitle_font, title_end_y + _BLOCK_GAP, highlight_color, uniform_rect_height
        )
...
def create_story(
    ...
    uniform_rect_height = _compute_uniform_rect_height(scratch, line_specs)
    ...
    if text_position == "top":
        block_rects, block_texts = _render_text_block(
            scratch, title_lines, subtitle_lines, title_font, subtitle_font, 110, highlight_color, uniform_rect_height
        )
